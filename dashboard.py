...
def dashboard_metric(key, val):
    try:
        if uvicorn_loop and not uvicorn_loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                manager.broadcast(
                    json.dumps({"type": "metric", "key": key, "value": val})
                ),
                uvicorn_loop,
            )
    except Exception:
    ...
    try:
        while True:
            try:
                ...
                print(tb)
                dashboard_logger(
                    f"[red]Acknowledger Crashed. Auto-restarting in 5s...\n{e}[/red]",
                    "ack",
                )
                import time
        ...
        if uvicorn_loop and not uvicorn_loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                manager.broadcast(
                    json.dumps({"type": "status_ack", "status": "stopped"})
                ),
                uvicorn_loop,
            )
        loop.close()
    ...
    try:
        while True:
            try:
                ...
                print(tb)
                dashboard_logger(
                    f"[red]Filler Crashed. Auto-restarting in 5s...\n{e}[/red]", "fill"
                )
                import time
        ...
        if uvicorn_loop and not uvicorn_loop.is_closed():
            asyncio.run_coroutine_threadsafe(
                manager.broadcast(
                    json.dumps({"type": "status_fill", "status": "stopped"})
                ),
                uvicorn_loop,
            )
        loop.close()
