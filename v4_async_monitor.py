...
def _intercept_print(*args, **kwargs):
    if args:
        ...
        try:
            safe_msg = msg.encode(sys.stdout.encoding or "ascii", "replace").decode(
...
async def async_input_with_timeout(prompt: str, timeout: int = 15) -> str:
    ...
    try:
        ...
        if notification:
            notification.notify(
                title="Manual Action Required", message=prompt, timeout=10
            )
        sys.stdout.write("\n")
        raise
...
async def close_side_panel(page):
    ...
    try:
        ...
        try:
            backdrop = (
                page.locator(".fixed.inset-0.bg-black, .bg-black.bg-opacity-50")
...
async def launch_or_connect(playwright):
    try:
        ...
        console.print(
            f"[green]Connected to your existing Chrome session directly![/green]"
        )
        return browser
    except Exception as e:
            ...
            try:
                ...
                if await label_loc.is_visible(timeout=500):
                    ...
                    lines = [
                        line
                        for line in lines
                        if not re.match(r"(?i)^(approved|acknowledged) by", line)
                    ]
                    if lines:
                        val = lines[0]
...
async def wait_for_success(page, action_name, tab_prefix=""):
    try:
        ...
        console.print(
            f"  {tab_prefix}[green]✓ Success banner seen for {action_name}[/green]"
        )
        return True
    except Exception:
        console.print(
            f"  {tab_prefix}[yellow]⚠ No success banner seen for {action_name}[/yellow]"
        )
        return False
...
async def paginate_or_refresh(page, bot_prefix=""):
        ...
        try:
            ...
            if await btn.is_visible(timeout=200):
                ...
                if not is_disabled:
                    if bot_prefix == "[Tab 2] ":
                        console.print(
                            f"[dim]{bot_prefix}Fast-forwarding to Next Page...[/dim]"
                        )
                    else:
                        console.print(
                            f"[dim]{bot_prefix}All incidents processed. Moving to Next Page...[/dim]"
                        )
                    await btn.click()
                    await asyncio.sleep(
                        5.0
                    )  # Increased to wait for React state to update
                    return True
        except Exception:
            continue
    ...
    console.print(
        f"[dim]{bot_prefix}End of queue. Refreshing to jump back to Page 1...[/dim]"
    )
    try:
        # Some dashboards have a "Refresh" button, we try clicking that first
        refresh_btn = (
            page.locator(
                'button:has-text("Refresh"), button[title*="Refresh"], .lucide-refresh-cw'
            )
            .locator("visible=true")
            .first
        )
    ...
    if match:
        ...
        if id_match:
            ...
            try:
                ...
                age_minutes = abs(
                    (datetime.utcnow() - incident_time).total_seconds() / 60
                )
            except Exception:
                age_minutes = 0
        else:
...
async def ack_worker(page):
    ...
    if saved_time:
        console.print(
            f"[Tab 1] [cyan]Resuming from last seen incident time: {saved_time}[/cyan]"
        )
    ...
    async def table_has_changed():
        ...
        try:
            changed = await page.evaluate(
                "() => { const v = window.__tableChanged; window.__tableChanged = false; return v; }"
            )
            return bool(changed)
        except Exception:
            return False
    ...
    async def scan_and_ack():
        ...
        for i in range(rows_count):
            try:
                ...
                unique_id, age_minutes, incident_time_str = parse_incident_age(
                    text, first
                )
                ...
                ack_btn = (
                    page.locator(
                        'button:has-text("Acknowledge"), button:has-text("Assign to me")'
                    )
                    .locator("visible=true")
                    .last
                )
    ...
    while True:
        try:
            ...
            if did_work:
                last_reload = (
                    time.time()
                )  # Reset reload timer so we don't interrupt active work
        ...
        try:
            ...
            for i in range(rows_count):
                try:
                    ...
                    modal_indicator = (
                        page.locator(
                            '[role="dialog"], .fixed.inset-y-0.right-0, .lucide-x'
                        )
                        .locator("visible=true")
                        .first
                    )
                    if not await modal_indicator.is_visible(timeout=25000):
                        console.print(
                            f"[Tab 2] [dim]Modal didn't seem to open for {first}, trying click again...[/dim]"
                        )
                        try:
                            await link.click(timeout=5000, force=True)
                            ...
                            console.print(
                                f"[Tab 2] [red]Row click failed: {click_err}[/red]"
                            )

                        # If it STILL didn't open, handle the failure
                        if not await modal_indicator.is_visible(timeout=25000):
                            incident_failure_counts[unique_id] = (
                                incident_failure_counts.get(unique_id, 0) + 1
                            )
                            if incident_failure_counts[unique_id] >= 2:
                                console.print(
                                    f"[Tab 2] [red]Cannot open modal for '{first}' after multiple retries. Permanently skipping this broken incident.[/red]"
                                )
                            ...
                            console.print(
                                f"[Tab 2] [dim]Skipping '{first}' for now. (Failure {consecutive_failures}/5)[/dim]"
                            )
                            temporarily_skipped_ids.add(unique_id)

                            if consecutive_failures >= 5:
                                console.print(
                                    "[Tab 2] [red]5 consecutive failures — Dashboard appears frozen! Reloading...[/red]"
                                )
                                await page.reload(wait_until="domcontentloaded")
                                try:
                                    await page.locator("table tbody tr").first.wait_for(
                    ...
                    try:
                        await page.locator('text="Root Cause Category"').last.wait_for(
                    ...
                    for v in scraped_data.values():
                        ...
                        if (
                            len(v_str) > 2
                            and "required if" not in v_str
                            and "alllowmediumhigh" not in v_str
                            and "select incident" not in v_str
                        ):
                            has_valid_data = True
                            break
                    ...
                    characteristic_key = (
                        re.sub(r"^[\d_-]+", "", characteristic_key).strip().lower()
                    )

                    if has_valid_data:
                        console.print(
                            f"[Tab 2] [cyan]Incident '{characteristic_key}' is already filled. Learning values...[/cyan]"
                        )
                        ...
                        if not is_garbage and "All" not in scraped_data.get(
                            "priority", ""
                        ):
                        ...
                        if isinstance(disk_memory, dict):
                            if characteristic_key in disk_memory:
                                ...
                                clean_key = characteristic_key.replace(
                                    "...", ""
                                ).lower()
                                for mem_key, mem_data in disk_memory.items():
                                    ...
                                    if clean_key.startswith(
                                        clean_mem
                                    ) or clean_mem.startswith(clean_key):
                        ...
                        if not found_in_disk:
                            ...
                            predicted_data, ai_error = (
                                await ai_agent.predict_incident_fields(
                                    characteristic_key,
                                    (
                                        disk_memory
                                        if isinstance(disk_memory, dict)
                            ...
                            if predicted_data:
                                console.print(
                                    f"[Tab 2] [green]AI Auto-Filled '{characteristic_key}'![/green]"
                                )
                                if metric_callback:
                                    metric_callback("ai_filled", 1)
                                ...
                                await save_to_memory(
                                    {characteristic_key: predicted_data}
                                )
                            else:
                                console.print(
                                ...
                                if log_callback:
                                    log_callback(
                                        f"[Tab 2] [red]AI Error: {ai_error}[/red]",
                                        "fill",
                                    )
                        ...
                        for field_key, field_value in fill_data.items():
                            if (
                                field_value
                                and len(str(field_value)) > 2
                                and "required if" not in str(field_value).lower()
                                and "alllowmediumhigh" not in str(field_value).lower()
                            ):
                                try:
                                    lbl_map = {
                                        "rc_description": "Root Cause Description",
                                    ...
                                    label_loc = (
                                        page.locator(f':text("{lbl}")')
                                        .locator("visible=true")
                                        .last
                                    )
                                    if not await label_loc.is_visible(timeout=3000):
                                        console.print(
                                            f"  [Tab 2] [red]✗ Label '{lbl}' not found on screen[/red]"
                                        )
                                        continue
                                    ...
                                    edit_btn = (
                                        parent_block.locator(edit_selectors)
                                        .locator("visible=true")
                                        .first
                                    )
                                    if not await edit_btn.is_visible(timeout=500):
                                        ...
                                        if not await edit_btn.is_visible(timeout=500):
                                            parent_block = label_loc.locator(
                                                "xpath=../../.."
                                            )
                                            edit_btn = (
                                                parent_block.locator(edit_selectors)
                                    ...
                                    for level in [
                                        ...
                                        test_loc = (
                                            label_loc.locator(f"xpath={level}")
                                            .locator(
                                    ...
                                    input_filled = False
                                    if input_loc:
                                        tag = await input_loc.evaluate(
                                            "el => el.tagName.toLowerCase()"
                                        )
                                        if tag == "select":
                                            # Native select
                                            try:
                                                await input_loc.select_option(
                                                    label=str(field_value), timeout=1000
                                                )
                                                input_filled = True
                                            except Exception:
                                                pass
                                    ...
                                    if not input_filled:
                                        ...
                                        for level in [
                                            ...
                                            trigger = (
                                                label_loc.locator(f"xpath={level}")
                                                .locator(
                                                    'button[aria-haspopup], div[class*="control"], .lucide-chevron-down'
                                                )
                                                .locator("visible=true")
                                                .first
                                            )
                                    ...
                                    if not input_filled:
                                        console.print(
                                            f"  [Tab 2] [red]✗ Input/Option not visible for {lbl}[/red]"
                                        )
                                        continue
                                    ...
                                    for level in [
                                        ...
                                        test_save = (
                                            label_loc.locator(f"xpath={level}")
                                            .locator(
                                                'button:has-text("Save"), :text-is("Save"), button.bg-blue-600'
                                            )
                                            .locator("visible=true")
                                            .first
                                        )
                                    ...
                                    if save_btn:
                                        ...
                                        console.print(
                                            f"  [Tab 2] [dim]Save button not found for {lbl} (assuming auto-save)[/dim]"
                                        )

                                except Exception as e:
                                    console.print(
                                        f"  [Tab 2] [red]Error on {field_key}: {str(e)[:100]}[/red]"
                                    )
                                    pass

                        if fields_successfully_filled == 0:
                            console.print(
                                "[Tab 2] [red]Failed to fill ANY fields! Aborting Finish Update.[/red]"
                            )
                            temporarily_skipped_ids.add(unique_id)
                            await close_side_panel(page)
                            continue
            ...
            if not processed_any:
                ...
                if moved_next:
                    ...
                    console.print(
                        "[Tab 2] [dim]Sweep complete. Looping back to Page 1...[/dim]"
                    )
                    current_page = 1
                    await asyncio.sleep(5)
        except Exception as e:
...
async def main_fill():
    async with async_playwright() as p:
        ...
        if len(pages) < 2:
            ...
            tab2 = pages[1]
            console.print(
                "[cyan]Using existing second tab for the Background Filler...[/cyan]"
            )
